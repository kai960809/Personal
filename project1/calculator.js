(() => {
  ...
  // Guard DOM elements to avoid runtime errors if script loads unexpectedly
  if (!display || !keys) {
    console.warn('Calculator: required DOM elements not found.');
    return;
  }
  let expr = '';
  let history = [];
  ...
  keys.addEventListener('click', (e) => {
    ...
    if (!action) {
      ...
      if (m) {
        ...
        // nothing, prepend a minus (will be interpreted as unary by the tokenizer)
        pushToken('-');
      }
    }
